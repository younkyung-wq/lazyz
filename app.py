...
function renderPage(){
  ...
  imgs.forEach((o,i)=>{
    ...
    pg.appendChild(row); o.el=row; drawRow(o); if(/누끼/.test(o.name) && !/누끼[\-_ ]*[A-Za-z]+\d/.test(o.name)){ const cn=niColorName(o.name); if(cn){ const cd=document.createElement('div'); cd.className='colorcap'; cd.textContent=(cn.charAt(0).toUpperCase()+cn.slice(1).toLowerCase()); row.appendChild(cd); placeCap(cd,cv,o); } } if(/누끼/.test(o.name)){ const _nx=imgs[i+1]; const _pv=imgs[i-1]; const _isNum=n=>/누끼[\-_ ]*[A-Za-z]+\d/.test(n); if(!_nx || !/누끼/.test(_nx.name) || baseCode(_nx.name)!==baseCode(o.name)){ row.style.marginBottom='150px'; } else if(!_isNum(o.name) && _isNum(_nx.name)){ row.style.marginBottom='130px'; } else { row.style.marginBottom='10px'; } if(_pv && !/누끼/.test(_pv.name)){ row.style.marginTop='150px'; } }
  });
  // 텍스트 섹션들

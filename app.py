...
.panel{width:300px;flex-shrink:0;height:812px;min-height:0;background:#fff;border-left:1px solid #e5e5e5;order:2;padding:10px 20px;display:flex;flex-direction:column;gap:14px;overflow-y:auto;}
.panel>*{flex-shrink:0;}
...
.optrow input{width:15px;height:15px;cursor:pointer;accent-color:#3a3a3a;}
...
.acc>summary::after{content:'▸';color:#999;}
...
.imgrow .colorcap{text-align:center;font-size:30px;font-weight:400;letter-spacing:0.02em;color:#222;margin-top:-50px;padding:0 0 40px;font-family:'Pretendard',-apple-system,sans-serif;position:relative;z-index:1;}
...
function codeOfColor(c){ const en=(c.en||'').toString().toUpperCase().trim(); for(const code in CODE_NAME){ if(CODE_NAME[code]===en) return code; } return ''; }
function colorCodeOf(fname){ const f=(fname||'').toUpperCase(); for(const code in CODE_NAME){ if(new RegExp('(^|[^A-Z])'+code+'([^A-Z]|$)').test(f)) return code; } return ''; }
// 누끼 아래 투명여백 비율 측정(0~1) — 제품 실제 밑끝 기준으로 컬러텍스트 간격 통일용
function nukiBottomFrac(o){ if(o._botFrac!=null) return o._botFrac; let frac=0; try{ const img=o.img; const w=Math.min(220, img.width||220); const h=Math.max(1,Math.round(w*(img.height/img.width))); const c=document.createElement('canvas'); c.width=w; c.height=h; const g=c.getContext('2d'); g.drawImage(img,0,0,w,h); const d=g.getImageData(0,0,w,h).data; let last=-1; for(let y=h-1;y>=0;y--){ let hit=false; for(let x=0;x<w;x+=2){ if(d[(y*w+x)*4+3]>10){ hit=true; break; } } if(hit){ last=y; break; } } frac = last<0 ? 0 : (h-1-last)/h; }catch(e){ frac=0; } o._botFrac=frac; return frac; }
function placeCap(cd, cv, o){ requestAnimationFrame(()=>{ try{ const fB=nukiBottomFrac(o); const hh=cv.offsetHeight||cv.getBoundingClientRect().height||0; const GAP=100; cd.style.marginTop=Math.round(GAP - fB*hh)+'px'; }catch(e){} }); }
...
function renderPage(){
  ...
  imgs.forEach((o,i)=>{
    ...
    pg.appendChild(row); o.el=row; drawRow(o); if(/누끼/.test(o.name) && !/누끼[\-_ ]*[A-Za-z]+\d/.test(o.name)){ const cn=niColorName(o.name); if(cn){ const cd=document.createElement('div'); cd.className='colorcap'; cd.textContent=(cn.charAt(0).toUpperCase()+cn.slice(1).toLowerCase()); row.appendChild(cd); placeCap(cd,cv,o); } } if(/누끼/.test(o.name)){ const _nx=imgs[i+1]; const _pv=imgs[i-1]; const _isNum=n=>/누끼[\-_ ]*[A-Za-z]+\d/.test(n); if(!_nx || !/누끼/.test(_nx.name) || baseCode(_nx.name)!==baseCode(o.name)){ row.style.marginBottom=(_nx && /앞|뒤/.test(_nx.name)) ? '200px' : '150px'; } else if(!_isNum(o.name) && _isNum(_nx.name)){ row.style.marginBottom='150px'; } else { row.style.marginBottom='10px'; } if(_pv && !/누끼/.test(_pv.name)){ row.style.marginTop='150px'; } }
  });
  // 텍스트 섹션들
...
const GUIDE_IMG=new Image(); GUIDE_IMG.crossOrigin='anonymous'; GUIDE_IMG.src='https://raw.githubusercontent.com/younkyung-wq/lazyz/main/guide.png';
function drawGuide(g,W,H){ if(GUIDE_IMG.complete && GUIDE_IMG.naturalWidth){ g.save(); g.globalAlpha=0.9; g.drawImage(GUIDE_IMG,0,0,W,H); g.restore(); } }
function makeThumb(){ if(!pendingImg||!pendingCrop) return pendingSrc; const W=200,H=245; const cv=document.createElement('canvas'); cv.width=W;cv.height=H; const g=cv.getContext('2d'); g.imageSmoothingQuality='high'; g.fillStyle='#f0f0f0'; g.fillRect(0,0,W,H); const img=pendingImg,t=pendingCrop; const base=Math.max(W/img.width,H/img.height); const z=base*t.z; const iw=img.width*z,ih=img.height*z; let dx=W/2-t.cx*iw,dy=H/2-t.cy*ih; dx=Math.min(0,Math.max(W-iw,dx)); dy=Math.min(0,Math.max(H-ih,dy)); g.drawImage(img,dx,dy,iw,ih); return cv.toDataURL('image/jpeg',0.85); }
...
function confirmAdd(){ if(!pendingSrc)return; const name=(document.getElementById('mName').value||'').trim(); const cap=(document.getElementById('mCap').value||'').trim(); const _thumb=makeThumb(); presets.push({id:newId(),name,cap,src:pendingSrc,crop:pendingCrop||{z:1,cx:0.5,cy:0.5},thumb:_thumb}); savePresets(); pendingSrc=null; pendingImg=null; pendingCrop=null; renderModelUI(); }
...
function renderModelUI(){
  ...
  if(pl) pl.innerHTML = presets.length ? presets.map(m=>{ const on=usedIds.includes(m.id);
    return '<div class="prow"><img src="'+(m.thumb||m.src)+'"><div class="pc">'+esc(m.name||'(이름 없음)')+'</div><button class="puse'+(on?' on':'')+'" onclick="toggleUse(\''+m.id+'\')">'+(on?'사용중':'사용')+'</button><button class="pdel" onclick="delPreset(\''+m.id+'\')">×</button></div>';
...
function sectionsHTML(){
  ...
  +modelsHTML()
  +'<div class="sec"><h2>Care</h2><div class="fab">Fabric : <span data-k="fabric" contenteditable>'+esc(P.fabric)+'</span><div style="margin-top:48px;">- 제조국 : <span data-k="made" contenteditable>'+esc(P.made||'국내')+'</span></div></div><div class="k" data-k="care" contenteditable>'+esc(P.care)+'</div></div>'
...
.mR .title{margin-top:200px;font-family:'Pretendard',-apple-system,sans-serif;font-size:24pt;font-weight:800;line-height:1.45;}
.mR .title .ten{letter-spacing:0;}
.mR .title .tko{letter-spacing:-0.04em;}
.mR .desc{margin-top:38px;font-size:21px;color:#333;line-height:1.5;}
.mR .desc div{margin-bottom:6px;}
...
function renderNukis(list){
  ...
  const H=Math.min(NK_MAXH,(NK_AVAILW-NK_GAP*(n-1))/sumA);
  metas.forEach((m,i)=>{
    const cn=(colorNameForCode(colorCodeOf(m.o.name))||'').toUpperCase();
    ...
    item.appendChild(makeNukiCanvas(m.o.img,m.bb,H));

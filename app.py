...
function renderPage(){
  ...
  imgs.forEach((o,i)=>{
    ...
    pg.appendChild(row); o.el=row; drawRow(o); if(/누끼/.test(o.name) && !/누끼[\-_ ]*[A-Za-z]+\d/.test(o.name)){ const cn=niColorName(o.name); if(cn){ const cd=document.createElement('div'); cd.className='colorcap'; cd.textContent=(cn.charAt(0).toUpperCase()+cn.slice(1).toLowerCase()); row.appendChild(cd); } } if(/누끼/.test(o.name)){ const _nx=imgs[i+1]; if(!_nx || !/누끼/.test(_nx.name) || baseCode(_nx.name)!==baseCode(o.name)){ row.style.marginBottom='100px'; } const _pv=imgs[i-1]; if(_pv && !/누끼/.test(_pv.name)){ row.style.marginTop='100px'; } }
  });
  // 텍스트 섹션들
...
function selectProduct(i){ const p=PRODUCTS[+i]; if(!p)return; if(curProdIdx!==-1 && +i!==curProdIdx){ prodImgs[curProdIdx]=imgs.slice(); const _fp=document.getElementById('folderpath'); if(_fp) prodFolder[curProdIdx]=_fp.value; imgs.length=0; const _rest=prodImgs[+i]||[]; for(const _o of _rest) imgs.push(_o); if(_fp) _fp.value=prodFolder[+i]||''; } curProdIdx=+i; P.name_en=p.name_en; P.desc=p.desc; P.sizeItems=p.sizeItems; P.sizeVals=p.sizeVals; P.sizeNote=p.sizeNote; P.fabric=p.fabric; P.made=p.made; P.colors=(p.colors||[]); const filt=PRODUCTS.map((x,j)=>({x,j})).filter(o=>prodFilter==='ALL'||o.x.shoot===prodFilter); const pos=filt.findIndex(o=>o.j===+i); const _c=document.getElementById('prodcount'); if(_c)_c.textContent=(pos+1)+' / '+filt.length; renderPage(); }
let prodFilter='ALL';

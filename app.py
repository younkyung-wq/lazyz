...
function drawGuide(g,W,H){ g.save(); g.strokeStyle='rgba(60,60,60,0.30)'; g.lineWidth=1.2; g.setLineDash([4,4]); g.beginPath(); g.moveTo(W/2,0); g.lineTo(W/2,H); g.stroke(); g.setLineDash([]); const hx=W/2,hy=H*0.30,hrx=W*0.155,hry=H*0.135; g.beginPath(); g.ellipse(hx,hy,hrx,hry,0,0,Math.PI*2); g.stroke(); g.beginPath(); g.moveTo(hx-W*0.09,hy+hry*0.85); g.quadraticCurveTo(hx-W*0.33,H*0.62,hx-W*0.44,H); g.moveTo(hx+W*0.09,hy+hry*0.85); g.quadraticCurveTo(hx+W*0.33,H*0.62,hx+W*0.44,H); g.stroke(); g.restore(); }

...
if "스토리 모듈" in menu:
    ...
    import re as _re
    st.markdown("<div style='font-size:16px;font-weight:800;color:#111;'>상세 생성기</div>", unsafe_allow_html=True)
    ...
    if loadbtn:
        ...
        if not p or not os.path.isdir(p):
            st.session_state.detail_err="경로가 올바르지 않아요. (로컬 실행 + NAS 마운트 필요)"
            st.session_state.detail_img=None
        else:
            ...
            if not files:
                st.session_state.detail_err="폴더에 이미지가 없어요."; st.session_state.detail_img=None
            else:
                W=1000; GAP=10
                ims=[]
                for f in files:
                ...
                totalH=sum(i.height for i in ims)+GAP*(len(ims)-1)
                canvas=Image.new("RGB",(W,totalH),"white")
                y=0
                for i in ims: canvas.paste(i,(0,y)); y+=i.height+GAP
                buf=io.BytesIO(); canvas.save(buf,"JPEG",quality=95)
                st.session_state.detail_img=buf.getvalue()
                st.session_state.detail_name=os.path.basename(p)
                st.session_state.detail_err=None

    if savebtn and st.session_state.get("detail_img"):
        st.download_button("📥 다운로드", st.session_state.detail_img,
                           file_name=(st.session_state.get("detail_name","상세페이지")+".jpg"),
                           mime="image/jpeg")

    with col1:
        st.markdown("**미리보기**")
        if st.session_state.get("detail_err"):
            st.error(st.session_state.detail_err)
        if st.session_state.get("detail_img"):
            st.image(st.session_state.detail_img, use_container_width=True)
        else:
            st.markdown("<div style='height:400px;background:#f2f2f2;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#bbb;'>경로 입력 후 불러오기</div>", unsafe_allow_html=True)
